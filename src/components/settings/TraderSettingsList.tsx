...
export function TraderSettingsList({ users }: { users: TraderSettingsUser[] }) {
  return (
    <section className="desk-surface overflow-hidden">
      <div className="border-b border-[var(--line)] px-5 py-5">
        <p className="terminal-label">Traders</p>
        <h2 className="mt-2 text-[15px] font-semibold text-[#e6eae7]">User names</h2>
      </div>

      {users.length === 0 ? (
        <div className="px-5 py-8 text-center text-[12px] text-[#78827e]">No users found. Run the seed.</div>
      ) : (
        <div className="divide-y divide-[var(--line)]">
          {users.map((user) => <TraderNameForm key={user.id} user={user} />)}
        </div>
      )}
    </section>
  );
...
function TraderNameForm({ user }: { user: TraderSettingsUser }) {
