  ...
  return (
    <section className="desk-surface overflow-hidden">
      <div className="border-b border-[var(--line)] px-5 py-5">
        ...
        <h2 className="mt-2 text-[15px] font-semibold text-[#e6eae7]">{isFr ? "Noms des utilisateurs" : "User names"}</h2>
      </div>

      {users.length === 0 ? (
        <div className="px-5 py-8 text-center text-[12px] text-[#78827e]">{isFr ? "Aucun utilisateur trouve. Lance le seed." : "No users found. Run the seed."}</div>
      ) : (
